...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <h4 class="mt-2">Edit Post</h4>
            <?= $this->include('layouts/includes/messages') ?>
            <div class="row">
                <form action="<?= base_url('posts/'.$post->id) ?>" method="POST" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    ...
                    <div class="form-floating-select">
                        ...
                        <select class="form-select <?= getErrorClass($validation, 'id_category') ?>" id="id_category" name="id_category" type="select" placeholder="Select category...">
                            ...
                            <?php foreach ($categories as $category) : ?>
                                <option value="<?= $category->id ?>" <?= $category->id == $post->id_category ? 'selected' : '' ?> ><?= $category->name ?></option>
                            <?php endforeach; ?>
                        </select>
                        <div class="invalid-feedback"><?= getErrorMessage($validation, 'id_category') ?></div>
                    </div>
                    <div class="form-floating">
                        <input class="form-control <?= getErrorClass($validation, 'title') ?>" id="title" name="title" type="text" value="<?= $post->title ?>" placeholder="Enter your title..." />
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 my-3">
                            <img src="<?= base_url($post->header_image) ?>" id="preview_image" class="img-thumbnail" width="250px">
                        </div>
                    </div>
                    <div class="mt-4">
                        <label for="post_content" style="color: #6c757d;">Post Content</label>
                        <textarea class="form-control <?= getErrorClass($validation, 'post_content') ?>" id="post_content" name="post_content" placeholder="Enter your post content..."><?= $post->post_content ?></textarea>
                        <div class="invalid-feedback"><?= getErrorMessage($validation, 'post_content') ?></div>
                    </div>
                    <button class="btn btn-primary text-uppercase mt-3 mb-3" id="submit-btn" type="submit">Update</button>
                </form>
            </div>
        </div>
    </div>
